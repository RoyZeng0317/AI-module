...
// OAuth 2.0 服務。GOOGLE_CLIENT_ID 是在 Google Cloud Console 申請的
// OAuth 用戶端 ID（Web application 類型），這是公開資訊、不是密鑰，可以
// 直接寫在前端；真正的安全邊界在後端 web/backend/auth.py 用同一組 ID 當
// audience 驗證 ID token 的簽章。三個地方（這支檔案、
// web/admin/frontend/src/components/google-auth-init.js、
