"""Unit tests for conversation_store.py — the persistence behind action.py
側邊欄「對話紀錄」清單. Each test uses its own tmp_path file so runs don't
touch the real web/backend/data/conversations.json.
"""

from web.backend.conversation_store import (
    DEFAULT_TITLE,
    append_message,
    clear_messages,
    create_conversation,
...
def test_append_message_omits_persona_and_mode_when_not_given(tmp_path):
    # 網頁端不帶 persona/mode（只有 GUI/CLI 呼叫這支共用模組時才會帶，見
    ...
    assert "persona" not in updated["messages"][0]
    assert "mode" not in updated["messages"][0]
