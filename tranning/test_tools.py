...
global (tools._last_quiz_problem, shared by the calculus / math word
problem / logic reasoning quiz domains — see test_tools_math_logic.py for
the latter two) — see tools.py's comment on why a plain global is enough
for this single-user desktop app. Because it's module-level state shared
across tests, every test that depends on it starting empty resets it
explicitly first, rather than relying on test execution order.
"""
...
def test_route_reply_quiz_request_returns_question_only_and_stores_state():
    ...
    assert tools._last_quiz_problem["topic"] == "derivative"
